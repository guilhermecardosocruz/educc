...
function formatBR(iso?: string | null): string | null {
  ...
  if (isNaN(d.getTime())) return iso ?? null;
...
function sanitizeFilename(name: string) {
  return (name || "aluno").normalize("NFKD")
    .replace(/[\u0300-\u036f]/g, "")
...
async function buildSingleStudentPDF(ev: EventPayload, st: Student): Promise<Uint8Array> {
  ...
  if (linhaData) drawRightAlignedText(page, linhaData, bottomLimit + 14 + bodyLH, 12, font, right, rgb(0.25,0.25,0.25));

  const lineY = 86;
  const cols = [
    { name: (ev.sign1_name ? ev.sign1_name.toUpperCase() : s1name), role: s1role },
    { name: (ev.sign2_name ? ev.sign2_name.toUpperCase() : s2name), role: s2role },
...
export async function POST(req: NextRequest, { params }: { params: { id: string }}) {
  try {
    ...
    const alunos: Student[] = Array.isArray(body?.students) ? body.students : [];
    ...
    const validos = alunos.filter(a => a?.aluno_nome);
    if (validos.length === 0) {
    ...
    return NextResponse.json({ ok: false, error: e?.message || "Falha ao gerar ZIP de certificados" }, { status: 500 });
  }
}
